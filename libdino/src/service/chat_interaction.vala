...
public class ChatInteraction : StreamInteractionModule, Object {
    ...
    public int get_num_unread(Conversation conversation) {
        ContentItem? content_item_read_up_to = stream_interactor.get_module(ContentItemStore.IDENTITY).get_item(conversation, 1, conversation.read_up_to.id);
        if (content_item_read_up_to == null) return 0;

        int num_unread = stream_interactor.get_module(ContentItemStore.IDENTITY).get_after(conversation, content_item_read_up_to, 51).size;
        if(num_unread == 51) return -1; // the number of unread items is > 50
        return num_unread;
    }
