...
private class DBusStatusNotifierItem : Object {
    ...
    [DBus (name = "ProvideXdgActivationToken")]
    public void ProvideXdgActivationToken(string token) { item.provide_xdg_activation_token(token); }
    [DBus (name = "Scroll")]
...
#if WINDOWS
[CCode (cname = "getpid", cheader_filename = "process.h")]
#else
...
public class StatusNotifierItem : Object {
    ...
    public signal void provide_xdg_activation_token(string token);
    ...
    private string name { owned get { return @"$id-$(getpid())-$registration_id"; } }
    private bool _on_tray_appeared_locked;
    ...
    private void on_bus_acquired(DBusConnection dbus_connection) {
        if(this.dbus_connection != null) {
            warning("dbus_connection re-acquired unexpectedly.");
            return;
        }
    ...
    private async void on_tray_appeared(DBusConnection conn, string watcher_name, string owner) {
        if(_on_tray_appeared_locked) return;
        _on_tray_appeared_locked = true;
        try {
            if(registered) {
                warning("Attempted to re-register StatusNotifierItem.");
                return;
            }
            ...
            _on_tray_appeared_locked = false;
        }
    }
    ...
    public void register() {
        ...
        name_owner_id = Bus.own_name(BusType.SESSION, name, BusNameOwnerFlags.NONE, on_bus_acquired, on_name_acquired,
